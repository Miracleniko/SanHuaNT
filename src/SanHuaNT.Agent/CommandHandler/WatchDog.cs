using SanHuaNT.Agent.Command;
using SanHuaNT.Log;

namespace SanHuaNT.Agent.CommandHandler;
    ...
    public virtual void CheckWatchDog()
    {
        foreach (var item in WatchDogs)
        {
            ...
            if (Service.Host is Systemd)
            {
                 ...
                 * 如果在服务器上安装SanHuaNT.Agent，启动时，将是以Systemd服务运行
                 * 如果在服务器上，再安装一个宝塔面板，在宝塔面板中，安装nginx、mysql、redis等等，都是以SysVinit服务运行
                 * 现在如果在SanHuaNT.Agent中，配置监控nginx、mysql、redis，由于主服务是以Systemd服务运行，
