...
using SanHuaNT.Data;
using SanHuaNT.Log;
using SanHuaNT.DbCode.Transform;
...
public partial class Area : Entity<Area>
{
    ...
    public override Boolean Valid(DataMethod method)
    {
        ...
        {
            var py = SanHuaNT.Common.PinYin.Get(Name);
            if (!py.IsNullOrEmpty()) PinYin = py;
        }
        if (JianPin.IsNullOrEmpty())
        {
            var py = SanHuaNT.Common.PinYin.GetFirst(Name);
            if (!py.IsNullOrEmpty()) JianPin = py;
        }

        // 坐标
        //if (Longitude != 0 || Latitude != 0) GeoHash = SanHuaNT.Data.GeoHash.Encode(Longitude, Latitude);
        if (method == DataMethod.Insert || Dirtys[nameof(Longitude)] || Dirtys[nameof(Latitude)])
        {
            if (Math.Abs(Longitude) > 0.001 || Math.Abs(Latitude) > 0.001)
                GeoHash = SanHuaNT.Data.GeoHash.Encode(Longitude, Latitude);
        }
