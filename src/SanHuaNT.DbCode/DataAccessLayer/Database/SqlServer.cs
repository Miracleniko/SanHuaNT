...
internal class SqlServer : RemoteDb
{
    ...
    protected override void OnSetConnectionString(ConnectionStringBuilder builder)
    {
        ...
        if (builder[Application_Name] == null)
        {
            ...
            builder[Application_Name] = $"SanHuaNT.DbCode_{name}_{ConnName}";
        }
    }
    #endregion
    ...
    public override SelectBuilder PageSplit(SelectBuilder builder, Int64 startRowIndex, Int64 maximumRows)
    {
        ...
        // 修复无主键分页报错的情况
        if (builder.Key.IsNullOrEmpty() && builder.OrderBy.IsNullOrEmpty()) throw new DbCodeException("分页算法要求指定排序列！" + builder.ToString());

        // 如果包含分组，则必须作为子查询
        var builder1 = builder.CloneWithGroupBy("SanHuaNT.DbCode_T0", true);
        ...
        var builder2 = builder1.AsChild("SanHuaNT.DbCode_T1", true);
        // 结果列处理
    ...
    public static String PageSplitByTopNotIn(String sql, Int64 startRowIndex, Int64 maximumRows, String? keyColumn)
    {
        ...
        if (tablename != sql)
            ...
            sql = $"({sql}) SanHuaNT.DbCode_Temp_a";
    ...
    public static String PageSplitMaxMin(String sql, Int64 startRowIndex, Int64 maximumRows, String keyColumn)
    {
        ...
        {
            if (maximumRows < 1)
                sql = $"Select * From {CheckSimpleSQL(sql)} Where {keyColumn}{(isAscOrder ? ">" : "<")}(Select {(isAscOrder ? "max" : "min")}({keyColumn}) From (Select Top {startRowIndex} {keyColumn} From {CheckSimpleSQL(sql)} Order By {keyColumn} {(isAscOrder ? "Asc" : "Desc")}) SanHuaNT.DbCode_Temp_a) Order By {keyColumn} {(isAscOrder ? "Asc" : "Desc")}";
            else
                sql = $"Select Top {maximumRows} * From {CheckSimpleSQL(sql)} Where {keyColumn}{(isAscOrder ? ">" : "<")}(Select {(isAscOrder ? "max" : "min")}({keyColumn}) From (Select Top {startRowIndex} {keyColumn} From {CheckSimpleSQL(sql)} Order By {keyColumn} {(isAscOrder ? "Asc" : "Desc")}) SanHuaNT.DbCode_Temp_a) Order By {keyColumn} {(isAscOrder ? "Asc" : "Desc")}";
            return sql;
        }
        return null;
    }
...
internal class SqlServerMetaData : RemoteDbMetaData
{
    ...
    public override String Backup(String dbname, String? bakfile, Boolean compressed)
    {
        ...
        if (!Path.IsPathRooted(bf)) bf = SanHuaNT.Setting.Current.BackupPath.CombinePath(bf).GetBasePath();
