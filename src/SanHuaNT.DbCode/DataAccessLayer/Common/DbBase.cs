...
abstract class DbBase : DisposeBase, IDatabase
{
    ...
    static DbBase()
    {
        ...
        var set = SanHuaNT.Setting.Current;
    ...
    protected void CheckConnStr()
    {
        if (ConnectionString.IsNullOrWhiteSpace())
            throw new DbCodeException("[{0}]未指定连接字符串！", ConnName);
    }
    ...
    public static DbProviderFactory? GetProviderFactory(String? name, String assemblyFile, String className, Boolean strict = false, Boolean ignoreError = false)
    {
        try
        {
            ...
            var plugin = SanHuaNT.Setting.Current.GetPluginPath();
            ...
            if (type == null) throw new DbCodeException("无法加载驱动[{0}]，请从nuget正确引入数据库驱动！", assemblyFile);
    ...
    /// <returns>如果是简单SQL语句则返回表名，否则返回子查询(sql) SanHuaNT.DbCode_Temp_a</returns>
    internal protected static String CheckSimpleSQL(String sql)
    {
        ...
        if (ms == null || ms.Count <= 0 || ms[0].Groups.Count < 2 ||
            String.IsNullOrEmpty(ms[0].Groups[1].Value)) return $"({sql}) SanHuaNT.DbCode_Temp_a";
    ...
    #region Sql日志输出
    /// <summary>是否输出SQL语句，默认为SanHuaNT.DbCode调试开关SanHuaNT.DbCode.Debug</summary>
