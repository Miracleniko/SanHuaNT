...
using SanHuaNT.DbCode.Exceptions;
...
internal abstract partial class DbSession : DisposeBase, IDbSession, IAsyncDbSession
{
    ...
    protected virtual DbCommand OnCreateCommand(String sql, CommandType type = CommandType.Text, params IDataParameter[]? ps)
    {
        ...
        if (timeout <= 0) timeout = DBCodeSetting.Current.CommandTimeout;
            ...
            foreach (DbRow dr in list)
            {
                if (dr.Table != dt)
                {
                    ...
                    foreach (var dc in columns)
                    {
                        ...
                        if (idx < 0) throw new DbCodeException($"DbTable中不存在名为[{dc.Name}/{dc.ColumnName}]的字段");
    ...
    public void WriteSQL(String sql)
    {
        ...
        // 如果页面设定有SanHuaNT.DbCode_SQLList列表，则往列表写入SQL语句
        DAL.LocalFilter?.Invoke(sql);

        if (!ShowSQL) return;

        var sqlpath = DBCodeSetting.Current.SQLPath;
