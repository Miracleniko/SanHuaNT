...
using SanHuaNT.DbCode.Common;
using SanHuaNT.DbCode.Configuration;
using SanHuaNT.DbCode.DataAccessLayer;
using SanHuaNT.DbCode.Model;
using SanHuaNT.DbCode.Shards;
...
public partial class Entity<TEntity> : EntityBase, IAccessor where TEntity : Entity<TEntity>, new()
{
        ...
        if (Meta.Table.DataTable.InsertOnly)
        {
            switch (method)
            {
                case DataMethod.Update:
                    throw new DbCodeException($"只写的日志型数据[{Meta.ThisType.FullName}]禁止修改！");
                case DataMethod.Delete:
                    throw new DbCodeException($"只写的日志型数据[{Meta.ThisType.FullName}]禁止删除！");
            }
        }
    ...
    public static SelectBuilder FindSQLWithKey(String? where = null)
    {
        ...
        if (uk == null) throw new DbCodeException("实体类缺少唯一主键字段，无法生成SQL语句！");
    ...
    private static SelectBuilder CreateBuilder(String? where, String? order, String? selects, Boolean needOrderByID)
    {
        ...
        // SanHuaNT.DbCode对于默认排序的规则：整型主键降序，其它情况默认
        // 返回所有记录
