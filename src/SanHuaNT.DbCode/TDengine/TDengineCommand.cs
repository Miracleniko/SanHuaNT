...
namespace SanHuaNT.DbCode.TDengine
{
    /// <summary>命令</summary>
    public class TDengineCommand : DbCommand
    {
        ...
        protected override DbDataReader ExecuteDbDataReader(CommandBehavior behavior)
        {
            ...
            if (!task.Wait(TimeSpan.FromSeconds(CommandTimeout))) throw new DbCodeException("执行超时");

            var handler = task.Result;
            var code = TD.ErrorNo(handler);
            if (handler == IntPtr.Zero) code = TD.ErrorNo(conn._handler);
            if (code != 0) throw new DbCodeException(TD.Error(handler) ?? $"执行出错[{code}]。");
