...
namespace SanHuaNT.DbCode.Common;
...
public class DataSimulation
{
    ...
    {
        var set = SanHuaNT.DbCode.DBCodeSetting.Current;
        set.TraceSQLTime = 0;

        var fact = Factory;
        var session = fact.Session;
        //var pst = SanHuaNT.DbCodeService.Container.ResolveInstance<IEntityPersistence>();
        var pst = fact.Persistence;
        var conn = fact.ConnName;

        // 关闭SQL日志
        //SanHuaNT.DbCode.Setting.Current.ShowSQL = false;
