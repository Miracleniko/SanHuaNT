...
using SanHuaNT.DbCode.Statistics;

namespace SanHuaNT.DbCode.Shards;
...
public class TimeShardPolicy : IShardPolicy
{
    ...
    public virtual ShardModel? Shard(Object value)
    {
        ...
        {
            if (Factory == null) throw new DbCodeException("分表策略要求指定实体工厂！");

            if (!Factory.Snow.TryParse(id, out var time, out _, out _) || time.Year <= 1970)
                throw new DbCodeException("雪花Id解析时间失败，无法用于分表");

            return Shard(time);
        }

        throw new DbCodeException($"分表策略无法识别数据[{value}]");
    }
    ...
    {
        var fi = GetField();
        if (fi == null) throw new DbCodeException("分表策略要求指定时间字段！");

        if (fi.Type == typeof(DateTime))
        {
            var time = entity[fi.Name].ToDateTime();
            if (time.Year <= 1970) throw new DbCodeException("实体对象时间字段为空，无法用于分表");
        ...
        throw new DbCodeException($"时间分表策略不支持[{fi.Type.FullName}]类型字段");
    }
    ...
    public virtual ShardModel? Shard(DateTime time)
    {
        ...
        var fi = GetField() ?? throw new DbCodeException("分表策略要求指定时间字段！");
    ...
    public virtual ShardModel[] Shards(Expression expression)
    {
        ...
        //if (exps.Count == 0) throw new SanHuaNT.DbCodeException($"分表策略要求查询条件包括[{fi}]字段！");
        ...
        throw new DbCodeException("分表策略因条件不足无法执行分表查询操作！");
    }
