...
using SanHuaNT.DbCode.Membership;

namespace SanHuaNT.DbCode.Model;
...
public class WhereBuilder
{
    ...
    private Expression ParseField(String exp)
    {
        ...
        throw new DbCodeException($"无法解析表达式[{exp}]");
    }
    ...
    private Model Parse(String exp, String[] seps)
    {
        foreach (var item in seps)
        {
            ...
            if (p >= 0)
            {
                ...
                if (Factory.Table.FindByName(name) is not FieldItem fi) throw new DbCodeException($"无法识别表达式[{exp}]中的字段[{name}]，实体类[{Factory.EntityType.FullName}]中没有该字段");
